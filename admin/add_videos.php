...
if(isset($_GET['vid']) && $_GET['vid']!=''){
	$video_required='';
	$vid = get_safe_value($con, $_GET['vid']);
	$res = mysqli_query($con, "select * from videos where vid='$vid'");
...
if(isset($_POST['submit'])){
	...
	$video_title = get_safe_value($con, $_POST['video_title']);
	$short_desc = get_safe_value($con, $_POST['short_desc']);
	
	$res = mysqli_query($con, "select * from videos where title='$video_title'");
	$check = mysqli_num_rows($res);
	if($check > 0){
		if(isset($_GET['vid']) && $_GET['vid']!=''){
    ...
    if ($msg == '') {
        if (isset($_POST['submit'])) {
            if (isset($_FILES['video']) && $_FILES['video']['name'] != '') {
                $videoTitle = $_POST['video_title']; // Assuming you have an input field for video title in your form
                $short_desc = $_POST['short_desc']; // Assuming you have an input field for video description
                $videoCategory = $_POST['categories_id']; // Assuming you have an input field for video category
    
                $video = rand(111111111, 999999999) . '_' . $_FILES['video']['name'];
                move_uploaded_file($_FILES['video']['tmp_name'], VIDEO_SERVER_PATH . $video);
    
                // Insert or update video details into the videos table
                if (isset($_GET['vid']) && $_GET['vid'] != '') {
                ...
                header('location: add_videos.php'); // Redirect to a page where you manage videos
                die();
            }
        }
    }
...
<div class="content pb-0">
    <div class="animated fadeIn">
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header"><strong>Video</strong><small> Form</small></div>
                    <form method="post" enctype="multipart/form-data">
                        <div class="card-body card-block">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="short_desc" placeholder="Enter video short description" class="form-control" required><?php echo $short_desc ?></textarea>
                            </div>
